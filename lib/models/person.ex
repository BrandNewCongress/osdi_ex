defmodule Osdi.Person do
  ...
  defp get_emails(%{email_addresses: address_structs}),
    do: address_structs |> Enum.map(& &1.address)
    ...
    case match(person) do
      ...
      existing = %Osdi.Person{id: _id} ->
        params =
          ...
          |> Map.put(
               :identifiers,
               combine_identifiers(person[:identifiers], existing.identifiers)
             )
